...
_temp_test_dir = None
_saved_working_dir = None


def setup():
    global _temp_test_dir
    global _saved_working_dir
    _saved_working_dir = os.getcwd()
    _temp_test_dir = tempfile.mkdtemp(prefix='grin-')
    os.chdir(_temp_test_dir)
...
@skipIf(sys.platform == 'win32', 'no AF_UNIX on win32')
def test_socket():
    fr = FileRecognizer()
...
@skipIf(sys.version_info < (3, 3) and sys.platform == 'win32',
        'Python 3.2+ is required for symlinks on Windows')
...
@skipIf(sys.platform == 'win32', 'Unix permissions model is not supported')
...
def test_walking():
    ...
    result = []
    for p, t in sorted(fr.walk('tree')):
        result.append((p.replace(os.sep, '/'), t))
...
def test_dot_dot():
    fr = FileRecognizer(skip_hidden_files=True, skip_hidden_dirs=True,
                        ...
                        skip_dirs=set(['skip_dir']))
    truth = [
        (os.path.join(os.pardir, 'binary'), 'binary'),
        (os.path.join(os.pardir, 'dir.skip_ext', 'text'), 'text'),
        (os.path.join(os.pardir, 'dir', 'subdir', 'text'), 'text'),
        (os.path.join(os.pardir, 'dir', 'text'), 'text'),
        (os.path.join(os.pardir, 'text'), 'text'),
        (os.path.join(os.pardir, 'text.dont_skip_ext'), 'text'),
    ]
